...
MBED_STORAGE_DEVICE_VENDOR_STRINGS = [
    "ven_mbed", "ven_segger", "ven_arm_v2m", "ven_nxp"
]
...
def _separate_composite_devices(usb_storage_devices):
    ...
    for vid_pid_path in usb_storage_devices:
        ...
        if len(vid_pid_components) != 2 and len(vid_pid_components) != 3:
            logger.debug(
                "Skipping USBSTOR device with unexpected VID/PID string format '%s'",
                vid_pid_path,
            )
            continue
    ...
    return non_composite_devices, composite_devices


def _gen_target_id_usb_id_mount_point_map(cached_mount_points, disks):
    ...
    for cached_mount_point_info in cached_mount_points:
        for index, disk in enumerate(copy(disks)):
            ...
            if match_string in cached_mount_point_info["volume_string"]:
                ...
                ] = cached_mount_point_info[
                    "mount_point"
                ]
                disks.pop(index)
                break
    return target_id_usb_id_mount_point_map


USB_KEY_STRING = "SYSTEM\\CurrentControlSet\\Enum\\USB"


def _gen_vid_pid_target_id_usb_id_map(vid_pid_entry_key_string_map, target_id_usb_ids):
    ...
    for vid_pid_path, entry_key_strings in vid_pid_entry_key_string_map.items():
        ...
        for target_id_usb_id in overlapping_target_id_usb_ids:
            composite_device_key_string = "%s\\%s" % (
                vid_pid_key_string, target_id_usb_id
            )
            ...
            vid_pid_target_id_usb_id_map[vid_pid_path][entry_key_string] = {
                "target_id_usb_id": target_id_usb_id, "is_prefix": is_prefix
            }
    return vid_pid_target_id_usb_id_map


def _update_important_bits(
    candidates,
    vid_pid_path,
    ...
    for composite_device_subdevice_number in range(MAX_COMPOSITE_DEVICE_SUBDEVICES):
        subdevice_type_key_string = "%s\\%s&MI_0%d" % (
            USB_KEY_STRING, vid_pid_path, composite_device_subdevice_number
        )
        try:
        ...
        for (
            entry_key_string, entry_data
        ) in entry_key_string_target_id_usb_id_map.items():
            cur_entry = candidates[entry_data["target_id_usb_id"]]
            if entry_data["is_prefix"]:
                prepared_entry_key_string = "%s&000%d" % (
                    entry_key_string, composite_device_subdevice_number
                )
            else:
                prepared_entry_key_string = entry_key_string
            subdevice_key_string = "%s\\%s" % (
                subdevice_type_key_string, prepared_entry_key_string
            )
            try:
            ...
            if capability == "msd":
                cur_entry["mount_point"] = target_id_usb_id_mount_point_map[
                ...
                try:
                    ...
                    logger.debug(
                        'Key "Device Parameters" not found under serial device entry'
                    )
                    continue

                try:
                    cur_entry["serial_port"], _ = winreg.QueryValueEx(
...
class MbedLsToolsWin7(MbedLsToolsBase):
    ...
    def find_candidates(self):
        ...
        target_id_usb_id_mount_point_map = _gen_target_id_usb_id_mount_point_map(
            cached_mount_points, disks
        )
        ...
        non_composite_devices, composite_devices = _separate_composite_devices(
            usb_storage_devices
        )
        ...
        vid_pid_target_id_usb_id_map = _gen_vid_pid_target_id_usb_id_map(
            vid_pid_entry_key_string_map, target_id_usb_ids
        )

        for (
            vid_pid_path, entry_key_string_target_id_usb_id_map
        ) in vid_pid_target_id_usb_id_map.items():
            _update_important_bits(
                candidates,
                vid_pid_path,
                entry_key_string_target_id_usb_id_map,
                target_id_usb_id_mount_point_map,
