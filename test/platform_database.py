...
from mbed_lstools.platform_database import (
    PlatformDatabase,
    DEFAULT_PLATFORM_DB,
...
class EmptyPlatformDatabaseTests(unittest.TestCase):
    ...
    def test_old_database(self):
        ...
        """
        with patch("mbed_lstools.platform_database.open") as _open, patch(
            "mbed_lstools.platform_database.getmtime"
        ) as _getmtime:
            stringio = MagicMock()
...
class OverriddenPlatformDatabaseTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pdb = PlatformDatabase(
            [self.overriding_db_path, self.base_db_path],
    ...
    def test_add_non_override(self):
        ...
        self.assertEqual(
            list(self.pdb.items()),
            [("1234", "Another_Platform"), ("0123", "Base_Platform")],
        )
    ...
    def test_load_override(self):
        ...
        """
        self.overriding_db.write(
            json.dumps(dict([("0123", "Overriding_Platform")])).encode("utf-8")
        )
        self.overriding_db.seek(0)
        self.pdb = PlatformDatabase(
            [self.overriding_db_path, self.base_db_path],
    ...
    def test_add_override_permanent(self):
        ...
        self.assertEqual(
            ...
            json.dumps(
                dict([("daplink", dict([("0123", "Overriding_Platform")]))])
            ).encode(
                "utf-8"
            ),
        )
...
class InternalLockingChecks(unittest.TestCase):
    ...
    def setUp(self):
        self.mocked_lock = patch(
            "mbed_lstools.platform_database.InterProcessLock", spec=True
        ).start()
